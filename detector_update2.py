...
import numpy as np
from numpy.core.fromnumeric import sort                             # Math
...
class Detector():
    def __init__(self, info, freq=FREQUENCY):
        ...
        self._odom_callback = rospy.Subscriber(ODOM_TOPIC, Odometry, self._odom_callback, queue_size=1)
    ...
    def _rgbd_callback(self, rgb_msg, depth_msg, cam_msg):
        ...
        try:
            ...
            area = [stats[i, cv2.CC_STAT_AREA] for i in range(num_labels)]
            sorted_area = sorted(area, reverse = True)
            sec_largest_component = sorted_area[1]
            index = area.index(sec_largest_component)
            x, y, w, h = [stats[index, stat] for stat in [cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT]]
           ...
           # x, y, w, h, area = [stats[, stat] for stat in [cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT, cv2.CC_STAT_AREA]]
            ...
            for i in range(1, num_labels):
                ...
                x, y, w, h, area = [stats[index, stat] for stat in [cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT, cv2.CC_STAT_AREA]]
                cX, cY = centroids[i]
